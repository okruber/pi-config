# Pi terminal UI for Pi

Date: 2026-09-04
Status: approved visual direction, pending implementation plan
Owning repo: `okruber/pi-config`
Target files: `themes/bone.json`, `themes/bone.terminal.yaml`, `extensions/omp-chatbox.ts`, `extensions/token-speed.ts`
Optional target: `extensions/pi-terminal-ui-header.ts`

## Problem

The current Pi interface uses the Parchment theme and an OMP-inspired editor status line. The interface is readable, but its colors do not match Pi Terminal UI's visual profile. The status line also uses a vivid rainbow that competes with the surrounding content.

The desired interface combines three references. Pi supplies the interaction model. OMP supplies the compact technical hierarchy. Pi Terminal UI supplies the palette and editorial tone.
...
1. Add an Pi Terminal UI light theme that maps the approved Bone Editorial direction to Pi's theme tokens.
2. Add a matching terminal-emulator theme because Pi cannot paint the terminal background.
3. Restyle the existing OMP chatbox with Pi Terminal UI colors while preserving its current behavior.
...
The palette was sampled from the supplied Pi Terminal UI header assets.
...
Color must reinforce meaning rather than decorate every field. The first-stage status line therefore replaces the current rainbow with a small set of semantic Pi Terminal UI colors.
...
Create `themes/bone.json` with all required Pi theme tokens. Reuse named variables from the Pi Terminal UI palette. Add compatibility aliases that existing extensions can resolve without special cases.
...
Create `themes/bone.terminal.yaml` in the same Warp-compatible format as `themes/parchment.terminal.yaml`. Use Bone as the background and Deep Navy as the foreground. Map ANSI colors to the nearest semantic Pi Terminal UI colors.
...
Replace the hardcoded rainbow with semantic Pi Terminal UI colors. Resolve colors from the active theme when possible. Keep direct truecolor fallbacks for themes that do not expose the expected variables.
...
Update `extensions/token-speed.ts` so its powerline resolves Pi Terminal UI variable names first. Keep its existing fallbacks for Catppuccin and Parchment. Preserve cache aggregation, token-rate calculation, layout, and status bridge behavior.
...
A minimal custom header is optional. It may identify `π pi-terminal-ui / pi` and show the Pi version in one or two terminal rows. It must not imitate a desktop title bar. It must not consume substantial transcript height.
...
After the files are installed, select `bone` through `/settings`. Import `themes/bone.terminal.yaml` into Orca and select it for the terminal. Run `/reload` after extension changes.
...
1. Status fields use Pi Terminal UI colors rather than the previous rainbow.
...
5. Token-speed color resolution prefers Pi Terminal UI variables and retains fallbacks.
