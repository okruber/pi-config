...
> Fork note: this fork renders the dialog inline instead of as an overlay, so the
> transcript is never covered and the overlay-stack details below don't apply.
> `Ctrl+]` still shrinks the dialog to one row and brings it back, routed through the
> focused component rather than the raw terminal listener. See the README.
